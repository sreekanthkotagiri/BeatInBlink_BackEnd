export const detectFont = (text: string): string => {
  if (/[ఁ-౿]/.test(text)) return 'telugu';           // Telugu
  if (/[ऀ-ॿ]/.test(text)) return 'devanagari';       // Hindi, Marathi, Sanskrit
  ...
  if (/[\u0A00-\u0A7F]/.test(text)) return 'gurmukhi'; // Punjabi (Gurmukhi)
  if (/[\u1100-\u11FF]/.test(text)) return 'korean';  // Hangul Jamo
  ...
  if (/[\u0600-\u06FF]/.test(text)) return 'arabic';  // Arabic
  if (/[\u0400-\u04FF]/.test(text)) return 'cyrillic'; // Russian, Ukrainian, etc.
  if (/[\u0100-\u024F]/.test(text)) return 'latin_extended'; // European Latin (Polish, Czech, etc.)
